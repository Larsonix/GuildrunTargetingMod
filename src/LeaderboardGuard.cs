...
// a run played with the mod on cannot submit a score and does not move the Red Rift win streak.
//
...
internal sealed class LeaderboardGuard : IModSwitch
{
    ...
    // The two the mod actually freezes.
    private static readonly string[] KnownStreakMethods =
    {
        "IncrementChallengeWinstreak", "ResetChallengeWinstreak"
    };

    // Streak-touching members that are known and DELIBERATELY NOT frozen. Kept apart from the list
    // above rather than merged into it, because "we patch this" and "we have looked at this and
    // chosen to leave it alone" are different claims and a single list would quietly assert the
    // first about both.
    //
    // UpdateChallengeWinstreak(ProgressionSaveDto) is the LOAD path: it fills the in-memory history
    // from the save at boot and recomputes the displayed streak. It appends no win and records no
    // loss, so there is nothing here for a modded run to launder. Freezing it would be worse than
    // useless - it would stop Leyline's own leaderboard-version reset from ever applying, which is
    // how a season rollover reaches an existing profile.
    //
    // It surfaces at all only because of an Il2CppInterop detail worth remembering: this method is
    // PRIVATE in the game's source, and the generated interop wrapper exposes it as public. So
    // "public instance" means something wider to a mod's reflection than it does in Leyline's code,
    // and any check of this shape will see members the game itself keeps to itself.
    private static readonly string[] StreakMethodsDeliberatelyNotFrozen =
    {
        "UpdateChallengeWinstreak"
    };
    ...
    /// <summary>True once both submission paths and both streak mutations are intercepted.</summary>
        ...
        MelonPreferences_Entry<bool> noticeShown)
    {
        ...
        try
        {
            ...
            // Named directly for the same early-loading reason as SteamPlatformService above.
            Type progressionType = typeof(Il2CppEmber.Scopes.Application.Progression.Services.ProgressionService);

            VerifyNoUnknownStreakMutation(progressionType);

            MethodInfo increment = FindStreakMutator(progressionType, "IncrementChallengeWinstreak");
            MethodInfo reset = FindStreakMutator(progressionType, "ResetChallengeWinstreak");
            ...
            PatchAndVerify(harmony, increment, nameof(IncrementChallengeWinstreakPrefix));
            PatchAndVerify(harmony, reset, nameof(ResetChallengeWinstreakPrefix));
            Applied = true;
            TryLogMessage("[TargetingMod] leaderboard suppression and Red Rift win-streak freeze applied");
        }
        catch (Exception e)
        {
            Applied = false;
            UnavailableReason = e.Message;
            TryLogError("[TargetingMod] leaderboard and streak guarantees could not be applied: " + e.Message);
        }
    }
    ...
    // A future public streak mutator would bypass both prefixes. Like an unknown submission path,
    // finding one switches the whole mod off, because all four patches are one guarantee.
    //
    // This test does NOT filter on parameter count, and that is the correction rather than an
    // oversight. It originally required no parameters, which made it blind to
    // IncrementChallengeWinstreak(Il2CppSystem.Guid) - the real member on the installed build, and
    // the very thing it exists to notice. It reported a clean bill on a type it could not see, and
    // a check that cannot see what it guards is worse than no check, because it is believed.
    // Match on the name and the void return only; the arity is Leyline's to change and has been.
    private static void VerifyNoUnknownStreakMutation(Type serviceType)
    {
        var unknown = new List<string>();
        const BindingFlags flags = BindingFlags.Public | BindingFlags.Instance;
        foreach (MethodInfo method in serviceType.GetMethods(flags))
        {
            if (!method.Name.EndsWith("ChallengeWinstreak", StringComparison.Ordinal)) continue;
            if (Array.IndexOf(KnownStreakMethods, method.Name) >= 0) continue;
            if (Array.IndexOf(StreakMethodsDeliberatelyNotFrozen, method.Name) >= 0) continue;
            if (method.ReturnType != typeof(void)) continue;
            unknown.Add(method.Name);
        }
        if (unknown.Count == 0) return;
        throw new InvalidOperationException(
            "the game can now change the Red Rift win streak by a route this mod does not freeze (" +
            string.Join(", ", unknown) + ")");
    }
    ...
    // Resolved by NAME and return type, deliberately without asserting a parameter list.
    //
    // The first version of this asked for a parameterless overload, because that is what the
    // decompiled tree shows. The installed build disagrees: Leyline moved the run's identity from
    // something the method generated for itself into something the caller supplies, so the real
    // member is IncrementChallengeWinstreak(Il2CppSystem.Guid). The lookup threw, the guard failed
    // closed, and the whole mod switched itself off on a healthy install. Pinning a parameter list
    // buys nothing here and costs exactly that.
    //
    // What still has to hold is that the name resolves to ONE member. An overload arriving later
    // would make "the streak mutator" ambiguous, and silently patching whichever came first is how
    // a guarantee turns into a coin flip, so ambiguity throws like absence does.
    private static MethodInfo FindStreakMutator(Type serviceType, string methodName)
    {
        ...
        throw new InvalidOperationException(
            serviceType.FullName + "." + methodName + " now has " + matches.Count +
            " void overloads, so which one changes the streak is ambiguous");
    }
    ...
    private static bool ChallengeScorePrefix()
    {
        try
        {
            LeaderboardGuard guard = _live;
    ...
    private static bool IncrementChallengeWinstreakPrefix()
    {
        try
        {
            LeaderboardGuard guard = _live;
            if (guard == null)
            {
                TryLogMessage("[TargetingMod] suppressed a Red Rift win-streak increment: the freeze is patched but has no policy behind it");
                return false;
            }
            // when the mod cannot tell, the streak must not go up.
            return guard.AllowStreakMutation("increment", false);
        }
        catch (Exception e)
        {
            TryLogError("[TargetingMod] suppressed a Red Rift win-streak increment because the freeze itself failed: " + e);
            return false;
        }
    }

    private static bool ResetChallengeWinstreakPrefix()
    {
        try
        {
            LeaderboardGuard guard = _live;
            if (guard == null)
            {
                TryLogMessage("[TargetingMod] allowed a Red Rift win-streak reset: the freeze is patched but has no policy behind it");
                return true;
            }
            // Allowing an uncertain reset can only send the streak to zero, never inflate it.
            return guard.AllowStreakMutation("reset", true);
        }
        catch (Exception e)
        {
            TryLogError("[TargetingMod] allowed a Red Rift win-streak reset because the freeze itself failed: " + e);
            return true;
        }
    }

    private bool AllowStreakMutation(string mutation, bool allowWhenCannotTell)
    {
        RunMarkState state = CurrentRunMarkState(out string why);
        if (state == RunMarkState.Marked)
        {
            TryLogMessage("[TargetingMod] suppressed a Red Rift win-streak " + mutation + ": " + why);
            return false;
        }
        if (state == RunMarkState.NotMarked)
        {
            TryLogMessage("[TargetingMod] allowed a Red Rift win-streak " + mutation + ": " + why);
            return true;
        }

        TryLogMessage("[TargetingMod] " + (allowWhenCannotTell ? "allowed" : "suppressed") +
                      " a Red Rift win-streak " + mutation +
                      ": whether this run is marked cannot be told (" + why + ")");
        return allowWhenCannotTell;
    }
    ...
    private bool AllowChallenge()
    {
        ...
        TryLogMessage("[TargetingMod] Red Rift streak submitted: this run was played without the mod");
        return true;
    }
    ...
    // The streak freeze cannot use the submission guard's two-valued, fail-closed question because
    // increment and reset have opposite safe answers. Inability to identify the run is therefore
    // a real third answer. The saved-run fallback covers main menu abandon: the game has one save,
    // calls Reset before deleting it, and the menu clears a stale mark whenever that save is gone.
    private RunMarkState CurrentRunMarkState(out string why)
    {
        why = null;
        string marked = _dirtyRunId.Value;
        if (string.IsNullOrEmpty(marked))
        {
            why = "there is no modded-run mark";
            return RunMarkState.NotMarked;
        }
        if (string.Equals(marked, UnidentifiedRun, StringComparison.Ordinal))
        {
            why = "the live mark says the mod ran in an unidentified run";
            return RunMarkState.Marked;
        }
        ...
        try
        {
            if (DataReaders.TryGet<RunSessionDataReader>(out var run) && run != null)
            {
                ...
                if (!string.IsNullOrEmpty(id))
                {
                    if (string.Equals(id, marked, StringComparison.OrdinalIgnoreCase))
                    {
                        why = "the live run id matches the modded-run mark";
                        return RunMarkState.Marked;
                    }
                    why = "the live run id does not match the modded-run mark";
                    return RunMarkState.NotMarked;
                }
                runFailure = "the live run has no id";
            }
            else
            {
                runFailure = "the live run session is unavailable";
            }
        }
        catch (Exception e)
        {
            runFailure = "reading the live run failed (" + e.Message + ")";
        }

        string registryFailure = null;
        PersistenceReader persistence = null;
        try
        {
            try { DataReaders.TryGet<PersistenceReader>(out persistence); }
            catch (Exception e) { registryFailure = "the persistence registry read failed (" + e.Message + ")"; }

            if (persistence == null)
            {
                MainMenuUIController controller = RuntimeDiscovery.FindLive<MainMenuUIController>();
                persistence = RuntimeDiscovery.ReadField<PersistenceReader>(controller, "_persistenceReader");
            }

            if (persistence != null)
            {
                if (persistence.HasGameRunMetadata)
                {
                    why = "the live mark and the one existing saved run identify the same run";
                    return RunMarkState.Marked;
                }
                why = "there is no saved run for the live mark";
                return RunMarkState.NotMarked;
            }

            why = runFailure + "; no persistence reader is available";
            if (registryFailure != null) why += "; " + registryFailure;
        }
        catch (Exception e)
        {
            why = runFailure + "; reading saved-run state failed (" + e.Message + ")";
        }

        TryLogError("[TargetingMod] cannot tell whether the current run is marked: " + why);
        return RunMarkState.CannotTell;
    }
    ...
    public string Diagnostic => !Applied
        ? "leaderboards and Red Rift streak NOT guarded (" + UnavailableReason + ")"
        : "SubmitScore, SubmitChallengeScore, IncrementChallengeWinstreak and " +
          "ResetChallengeWinstreak patched; marked run=" +
