...
internal sealed class MenuUI
{
    ...
    // rest of this file. Do not make these consistent with each other.
    private static bool Fr => IsFrench();

    private static string LabelOn => Fr ? "Mod de ciblage : Activé" : "Targeting Mod: On";
    private static string LabelOff => Fr ? "Mod de ciblage : Désactivé" : "Targeting Mod: Off";
    private static string LabelUnavail => Fr ? "Mod de ciblage : indisponible" : "Targeting Mod: unavailable";

    private static string TurnOnTitle => Fr ? "Activer le mod de ciblage ?" : "Turn on the targeting mod?";
    private static string TurnOnBody => Fr
        ? "Le mod montre où chaque unité va se déplacer et qui elle va attaquer, avant le début du combat. Tant qu'il est activé, vos scores ne sont pas envoyés aux classements. Une partie jouée avec le mod activé ne compte jamais, même si vous le désactivez ensuite."
        : "The mod shows where every unit will move and who it will attack, before the fight starts. While it is on, your scores are not sent to the leaderboards. A run played with the mod on never counts, even if you turn the mod off later.";

    private static string TurnOffTitle => Fr ? "Désactiver le mod de ciblage ?" : "Turn off the targeting mod?";
    private static string TurnOffBody => Fr
        ? "Le plateau redevient normal et vos scores sont de nouveau envoyés aux classements."
        : "The board goes back to normal and your scores are sent to the leaderboards again.";
    private static string TurnOffRunLine => Fr
        ? "La partie en cours a été jouée avec le mod activé : elle ne compte donc toujours pas. Commencez une nouvelle partie pour que vos scores comptent de nouveau."
        : "The run you have in progress was played with the mod on, so it still does not count. Start a new run for your scores to count again.";
    private static string TurnOffStreakLine => Fr
        ? "Votre série de victoires de la Faille Rouge contient aussi une partie jouée avec le mod activé : elle reste donc hors de ce classement jusqu'à ce que la série reparte de zéro."
        : "Your Red Rift win streak also contains a run played with the mod on, so it stays out of that leaderboard until the streak resets.";

    private static string NoticeTitle => Fr ? "Mod de ciblage" : "Targeting Mod";
    private static string NoticeBody => Fr
        ? "Le mod de ciblage est activé. Il montre où chaque unité va se déplacer et qui elle va attaquer, avant le début du combat. Tant qu'il est activé, vos scores ne sont pas envoyés aux classements. Vous pouvez le désactiver à tout moment depuis le menu principal."
        : "The targeting mod is on. It shows where every unit will move and who it will attack, before the fight starts. While it is on, your scores are not sent to the leaderboards. You can switch it off at any time from the main menu.";

    private static string UnavailableTitle => Fr ? "Mod de ciblage" : "Targeting Mod";
    private static string UnavailableBody => Fr
        ? "Le mod s'est désactivé lui-même : le jeu a changé et il ne peut plus garantir que vos scores resteraient hors des classements. Vérifiez s'il existe une mise à jour du mod."
        : "The mod has switched itself off because the game has changed and it can no longer be sure your scores would stay out of the leaderboards. Check for a mod update.";
    ...
    private void OnClicked()
    {
        try
        {
            ...
            if (_modSwitch.ChallengeStreakIsModded) body += " " + TurnOffStreakLine;
