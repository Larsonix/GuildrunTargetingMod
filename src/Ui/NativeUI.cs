...
internal sealed class NativeUI
{
    ...
    // Every word the player ever sees lives here, English and French side by side, so the two
    // cannot drift apart and nothing that computes a result also writes prose.
    //
    // The punctuation below follows the game, one language at a time, and not the habits of the
    // rest of this file. The English game writes "Difficulty: A" with no space before the colon,
    // and the French one writes "Niveau de difficulté : A" with one. Both are correct for their
    // language, and text that does not match reads as foreign sitting next to the game's own.
    // Do not make these consistent with each other.
    private static bool Fr => IsFrench();
    ...
    {
        ModNotice.Disagreement => Fr
            ? "Aperçu désactivé : le mod et le combat n'étaient pas d'accord. Il revérifie au prochain combat."
            : "Preview turned off: the mod and the fight disagreed. It checks again next battle.",
        ModNotice.ReadFailure => Fr
            ? "Aperçu désactivé : le mod n'a pas pu lire le combat correctement."
            : "Preview turned off: the mod could not read the battle correctly.",
        _ => null
    };
    ...
    private static string PreviewTooltipTitle => Fr ? "Aperçu de l'ouverture" : "Opening preview";
    private static string PreviewTooltipBody => Fr
        ? "Affiche tout le plateau d'un coup : où chaque unité s'arrête, et contre qui elle se bat une fois arrivée. Remis à zéro au début de chaque combat."
        : "Shows the whole board at once: where every unit ends up, and who it is fighting there. Starts off again at the beginning of each battle.";

    private static string OriginTooltipTitle => Fr ? "Flèches depuis les positions finales" : "Arrows from final positions";
    private static string OriginTooltipBody => Fr
        ? "Les flèches d'attaque partent de là où les unités seront. Désactive pour qu'elles partent de leur position actuelle."
        : "Attack arrows start where units will be standing. Turn it off to make them start where units are right now.";

    private static string MidHeadTooltipTitle => Fr ? "Repères de direction" : "Direction marks";
    private static string MidHeadTooltipBody => Fr
        ? "Ajoute une petite flèche au milieu de chaque ligne, pour voir dans quel sens elle va."
        : "Adds a small arrow in the middle of every line, so you can tell which way it goes.";

    private static string TransparencyTooltipTitle => Fr ? "Unités transparentes" : "See-through units";
    private static string TransparencyTooltipBody => Fr
        ? "Estompe les unités pour voir le plateau au travers, pendant tout le placement. Désactive pour les garder en couleurs pleines."
        : "Fades the units so you can see the board through them, for the whole placement. Turn it off to keep them at full colour.";

    private static string StillMovingText => Fr ? "Toujours en mouvement" : "Still moving";
    ...
    //
    // The French game translates the key name as well, writing "Ouvrir la Boutique (Espace)",
    // which is why the label comes from the keyboard the player is actually using.
    ...
    //
    // And a key is a position on the keyboard, not a letter. A, Q, Z, W and M all sit somewhere
    // else on the French keyboards this mod is translated for, so any of them would land under a
    // different finger there. P, F and D are in the same place on both. They happen to work as a
    // reminder in both languages too : P for preview and for apercu, F for from and for fleches,
    // D for direction either way.
    //
    ...
    // The label the player reads, taken from the keyboard they are actually using. On a French
    // keyboard that means the character their own key produces, not the English name of the
    // position. The plain name is only the fallback for a key the layout cannot name.
    ...
    private void PollShortcuts()
    {
        ...
        try
        {
            ...
            // T for transparency and for transparence, and it sits in the same place on the French
            // keyboards this mod is translated for, which is the rule the three above follow.
    ...
    // Follow the language the player chose in the game, not the one Windows is set to. The system
    // language is only the fallback for when the game has not settled on one yet.
    private static bool IsFrench()
    {
        try
        {
            string code = LocalizationSettings.SelectedLocale?.Identifier.Code;
            if (!string.IsNullOrEmpty(code)) return code.StartsWith("fr", StringComparison.OrdinalIgnoreCase);
        }
        catch { }
        return Application.systemLanguage == SystemLanguage.French;
    }
